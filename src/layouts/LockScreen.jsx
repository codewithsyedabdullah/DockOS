...
export default function LockScreen({ goNext, isLocked }) {
  ...
  const [lockscreenWallpaper, setLockscreenWallpaper] = useState(() => {
    return localStorage.getItem("lockscreen_wallpaper") || "/Wallpaper/26-Tahoe-Beach-Dawn-thumb.jpeg";
  });
  ...
  // User profile state
  const [username, setUsername] = useState(() => localStorage.getItem("lock_username") || "Syed Abdullah");
  ...
  useEffect(() => {
    const img = new Image();
    img.src = localStorage.getItem("lockscreen_wallpaper") || "/Wallpaper/26-Tahoe-Beach-Dawn-thumb.jpeg";
  }, []);
