...
export default function Finder({ initialPath = "/icloud", windowId, maximized, isDragging, isResizing }) {
  ...
  const saveTextFile = (fileId, newTitle, newText) => {
    ...
    setAbdullahFiles((prev) => (prev.some((f) => f.id === fileId) ? updateFilesList(prev) : prev));
  ...
  const [abdullahFiles, setAbdullahFiles] = useState(() => {
    const saved = localStorage.getItem("os_abdullah_files");
    const parsed = saved ? JSON.parse(saved) : [];
    return parsed.filter(f => f.id !== "abdullah_welcome");
  });
  ...
  useEffect(() => {
    localStorage.setItem("os_abdullah_files", JSON.stringify(abdullahFiles));
  }, [abdullahFiles]);
  ...
  useEffect(() => {
    ...
    const handleFilesSync = () => {
      ...
      const abdullah = JSON.parse(localStorage.getItem("os_abdullah_files") || "[]");
      ...
      setAbdullahFiles(prev => JSON.stringify(prev) === JSON.stringify(abdullah) ? prev : abdullah);
    };
  ...
  useEffect(() => {
    ...
    const handleFileRestored = (e) => {
      ...
      } else if (path === "/abdullah-sp") {
        setAbdullahFiles(updateList);
      }
    };
  ...
  const getPathDisplayName = (path) => {
    ...
    if (path === "/abdullah-sp") return "Syed Abdullah";
  ...
  const getFilesForCurrentPath = () => {
    ...
    if (currentPath === "/abdullah-sp") {
      return abdullahFiles;
    }
  ...
  const deleteFile = (fileId) => {
    ...
    targets.forEach(fileToTrash => {
      ...
      } else if (currentPath === "/abdullah-sp") {
        setAbdullahFiles(prev => prev.filter(f => f.id !== fileToTrash.id));
      }
    });
  ...
  const addFileToCurrentDirectory = (newFile) => {
    if (currentPath === "/icloud") {
      ...
      setAbdullahFiles(prev => [...prev, newFile]);
    }
  };
  ...
  const handlePaste = () => {
    ...
    if (currentIsCutMode) {
      clipboardList.forEach((clipboardItem) => {
        ...
        setAbdullahFiles(prev => removeFromFileList(prev));
        ...
        const abdullah = removeFromFileList(JSON.parse(localStorage.getItem("os_abdullah_files") || "[]"));
        ...
        localStorage.setItem("os_abdullah_files", JSON.stringify(abdullah));
  ...
  const handleRename = (file) => {
    ...
    showCustomPrompt("rename", "Rename Item", file.name, (newName) => {
      ...
      if (currentPath === "/icloud") {
        ...
        setAbdullahFiles(prev => prev.map(f => f.id === file.id ? updatedFile : f));
      }
        ...
        <nav className="flex-1 overflow-y-auto px-2 pb-4 space-y-4" style={{ scrollbarWidth: "none" }}>
          ...
          <div>
            ...
            <div className="mt-1 space-y-0.5">
              ...
              <button
                onClick={() => navigateTo("/abdullah-sp")}
                className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] font-medium transition-all ${
                  currentPath === "/abdullah-sp" 
                    ? (isDarkMode ? "bg-white/10 text-white" : "bg-black/10 text-black") 
                    : (isDarkMode ? "text-[#D0D0D0] hover:bg-white/5" : "text-[#4A4A4A] hover:bg-black/5")
                }`}
              >
                <Home size={16} className={isDarkMode ? "text-gray-400" : "text-gray-500"} />
                <span>Syed Abdullah</span>
              </button>
              <button
